...
const ChangePayoutMethod = ({route, navigation }) => {

    const {user_id, jwtToken} = route.params


  const [existingPayoutMethod, setExistingPayoutMethod] = useState('');  
...
if (paymentType === "Paypal") {
    ...
    try {
        ...
        const response = await fetch("http://"+ServerIP+":3000/ChangePayoutMethod", {
            method: 'POST',
    ...
    try {
        ...
        const response = await fetch("http://"+ServerIP+":3000/ChangePayoutMethod", {
            method: 'POST',
            headers: {'Content-Type' : 'application/json'},
            body: JSON.stringify({'payout_type' : paymentType, 'card_holder' : cardHolder,
             'card_number' : cardNumber, 'expiration_date' : expirationDate, 'cvv' : cvc, 'user_id' :user_id}),
        });
...
// Getting The Old PayoutMethod

useEffect(() =>{

    const GetOldPayoutMethod = async() =>{
        try {
            ...
            console.log(`This User Has a ${data.Existing_payout_Method} payout method`);
            setExistingPayoutMethod(data.Existing_payout_Method);
        } catch (error) {
            console.log(error)
        }
    }
  ...
  return (
      <View style={{ flex: 1, width: screenWidth, height: screenHeight }}>
          <View style={{ width: screenWidth, height: screenHeight / 7 }}>
                {existingPayoutMethod === "Paypal" ? (                    
              <View style={{flexDirection: 'row', alignItems: 'center', marginLeft: screenWidth/15, marginTop: screenHeight / 15 }}>
                    <Text style={{ fontSize: screenWidth / 15, fontFamily: 'Black', marginBottom: 16 }}>
                        You are Using {existingPayoutMethod}
                    </Text>
                    <Icon name='paypal' color={'rgb(65, 112, 200)'} size={screenWidth / 13} style={{ marginLeft: screenWidth / 30, marginBottom: screenHeight/50}} />
              </View>
                ) : (
                    <View style={{flexDirection: 'row', alignItems: 'center', marginLeft: screenWidth/20, marginTop: screenHeight / 15 }}>
                    <Text style={{ fontSize: screenWidth / 15, fontFamily: 'Black', marginBottom: 16 }}>
                        You are Using C Card
                    </Text>
                    <Icon name='credit-card' color={'rgb(65, 112, 200)'} size={screenWidth / 13} style={{ marginLeft: screenWidth / 30, marginBottom: screenHeight/50}} />
              </View>
                )}
          </View>
