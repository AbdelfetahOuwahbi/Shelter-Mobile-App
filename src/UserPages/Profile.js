...
export default function Profile({route, navigation}) {
    ...
    useEffect(() => {
        ...
        async function GetProfileContent() {
            ...
            if (data.message === "Here Are The User's Informations -->") {
                ...
                const date = data.date_joined.split("T")[0]
                const hour = data.date_joined.split("T")[1].split(".")[0]
                setDateJoined(`${date} at ${hour}`);
  ...
  useEffect(() =>{
    ...
    const CheckPayoutMethodExistence = async() =>{

        try {
            
            const response = await fetch("http://"+ServerIP+":3000/CheckPayoutExistence", {
                method: 'POST',
            ...
            if (data.message === "Payout Method Exists") {
            ...
            <ScrollView showsVerticalScrollIndicator={false}>
                ...
                <View style={{ height: screenHeight / 7.5, width: screenWidth }}>
                    <View style={{ alignItems: 'center', flexDirection: 'row', justifyContent: 'space-between' }}>
                        <TouchableOpacity style={{ marginTop: screenHeight / 18, marginLeft: screenWidth / 10 }} onPress={() => { navigation.navigate('Home', { user_id: user_id, jwtToken: jwtToken }) }}>

                            <Icon name='arrow-left' size={screenWidth / 14} color={'#C48F49'} />
                        </TouchableOpacity>

                        <TouchableOpacity style={{ marginTop: screenHeight / 18, marginRight: screenWidth / 10 }} onPress={() => { navigation.navigate('EditPersonalInfo', { user_id: user_id, jwtToken: jwtToken, Fname : first_name, Lname : last_name, Email : email, Phone : phone, Address : address}) }}>
                ...
                <TouchableOpacity onPress={() => { setShowCard(true); setCurrentImage("CoverPic"); }}>
                    <ImageBackground
                        source={coverPicture !== null ? { uri: coverPicture } : require('./UsrPgsImg/userGrey.png')}
                ...
                <View style={{ top: screenHeight / 90, alignItems: 'center', justifyContent: 'center', }}>
                    ...
                    <TouchableOpacity onPress={() => { setShowCard(true); setCurrentImage("ProfilePic"); }} style={{ marginRight: screenWidth / 2 }}>
                        <Image
                            source={profilePicture !== null ? { uri: profilePicture } : require('./UsrPgsImg/userGrey.png')}
                ...
                <View style={{ height: screenHeight / 1.5, flex: 1, width: screenWidth }}>
                    ...
                    <View style={{ marginLeft: screenWidth / 9 }}>
                        ...
                        <View style={{ marginTop: screenHeight / 20, flexDirection: 'column' }}>
                            <View style={{ flexDirection: 'row' }}>
                                <MaterialIcons name="update" size={24} color="#C48F49" />
                                <Text style={{ fontSize: screenWidth / 20, fontFamily: 'Black' }}>  Joined At :</Text>


                            </View>

                            <View style={{ marginTop: screenHeight / 30 }}>
                                <Text style={{ fontFamily: 'Bold', color: 'grey', fontSize: screenWidth / 28 }}>• {date_joined}</Text>
                            </View>
                        </View>
                    </View>
                ...
                <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-evenly', bottom: screenHeight / 15 }}>
                    {doesExist ? (
                    <TouchableOpacity onPress={() => {navigation.navigate('ChangePayoutMethod', {user_id : user_id, jwtToken : jwtToken})}} style={{ alignItems: 'center', justifyContent: 'center', borderColor: '#fff', backgroundColor: '#C48F49', borderRadius: screenWidth / 20, height: screenHeight / 24, width: screenWidth / 2 }}>
                    <Text style={{ fontSize: screenWidth / 28, color: '#fff', fontFamily: 'Black' }}>Change payout method</Text>
                </TouchableOpacity>
                    ) : (
                    <TouchableOpacity onPress={() => {navigation.navigate('AddPayoutMethod', {user_id : user_id, jwtToken : jwtToken})}} style={{ alignItems: 'center', justifyContent: 'center', borderColor: '#fff', backgroundColor: '#C48F49', borderRadius: screenWidth / 20, height: screenHeight / 24, width: screenWidth / 2.1 }}>
                        <Text style={{ fontSize: screenWidth / 28, color: '#fff', fontFamily: 'Black' }}>Add a payout method</Text>
                    </TouchableOpacity>
                    )}
                    {doesExist ? (
                    <TouchableOpacity onPress={() => {navigation.navigate('CreateItem', {user_id : user_id, jwtToken : jwtToken})}} style={{ alignItems: 'center', justifyContent: 'center', borderColor: '#fff', backgroundColor: '#C48F49', borderRadius: screenWidth / 20, height: screenHeight / 24, width: screenWidth / 2.1 }}>
                    <Text style={{ fontSize: screenWidth / 28, color: '#fff', fontFamily: 'Black' }}>Add New Housing</Text>
                </TouchableOpacity>
                    ) : (
                    <TouchableOpacity onPress={() => { navigation.navigate('CreateItem', { user_id: user_id, jwtToken: jwtToken }) }} style={{ alignItems: 'center', justifyContent: 'center', borderColor: '#fff', backgroundColor: '#C48F49', borderRadius: screenWidth / 20, height: screenHeight / 24, width: screenWidth / 2.2 }}>
                        <Text style={{ fontSize: screenWidth / 28, color: '#fff', fontFamily: 'Black' }}>Add New Housing</Text>
                    </TouchableOpacity>
                    )}
                </View>

                <TouchableOpacity onPress={HandleLogout} style={{alignItems:'center', marginTop:screenHeight/20, paddingBottom: screenHeight/60}}>
                    <View style={{flexDirection:'row'}}>
